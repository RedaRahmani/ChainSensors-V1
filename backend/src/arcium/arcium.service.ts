...
/**
 * ArciumService
 * - getMXEPublicKey(): fetch MXE X25519 pubkey from on-chain MXEAccount
...
export class ArciumService {
  ...
  private getProgramId(): PublicKey {
    // Prefer IDL address; fallback to Program.programId (they should match)
    const idlAddr = (idl as any)?.address;
    return new PublicKey(idlAddr || (this.program as any).programId);
  }

  /** Fetch MXE X25519 public key from Arcium program accounts */
  async getMxePublicKey(): Promise<Uint8Array> {
    ...
    if (!pub || pub.length !== 32) {
      throw new Error('Invalid MXE public key');
    }
    return pub;
  }
  ...
  /**
   * Placeholder for Phase 2 (MXE circuit): re-encrypt capsule for buyer.
   * We DO NOT have MXE private key here, so this must be performed by an MXE job.
   * Implement once `reencrypt_dek_for_buyer` circuit is deployed.
   */
