...
//    * 5) Return onboarding info
//    */
//   async enrollDevice(devicePubKey: string, metadata: EnrollMetadata) {
...
//     const ekHashArr = metadata.ekPubkeyHash ?? Array(32).fill(0);
//     const accessHashArr = metadata.accessKeyHash ?? Array(32).fill(0);
//     const ekHash = Uint8Array.from(ekHashArr);
//     const accessHash = Uint8Array.from(accessHashArr);
//     const deviceType = metadata.model;
//     const locationStr = `${metadata.location.latitude},${metadata.location.longitude}`;
//     const firstData = metadata.dataTypes[0] || { type: '', units: '', frequency: '' };
//     const dataType = firstData.type;
//     const dataUnit = firstData.units;
//     const pricePerUnit = metadata.pricePerUnit ?? 1;
//     const totalDataUnits = metadata.totalDataUnits ?? 1000;
...
//     return { deviceId, brokerUrl: this.brokerUrl, txSignature };
//   }

//   /**
//    * Update the lastSeen timestamp and latest data CID for a device
//    */
