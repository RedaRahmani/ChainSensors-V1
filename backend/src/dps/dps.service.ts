...
function envMultiline(name: string): string | undefined {
...
function makeEphemeralCa() {
  ...
  cert.validity.notAfter  = new Date(now.getTime() + 3650 * 24 * 60 * 60 * 1000); // ~10y
  const attrs = [{ name: 'commonName', value: 'ChainSensors Ephemeral CA' }];
...
export class DpsService {
  ...
  private readonly caKey: forge.pki.PrivateKey;
  private readonly caCert: forge.pki.Certificate;
  private readonly caIsEphemeral: boolean;
  ...
  constructor(
    ...
    const bu = this.configService.get<string>('BROKER_URL');
    if (!bu) throw new Error('BROKER_URL not set');
    this.brokerUrl = bu;

    // ---- Load CA from env (preferred), else auto-generate ephemeral CA
    const caKeyPemEnv  = envMultiline('CA_KEY_PEM');
    const caCertPemEnv = envMultiline('CA_CERT_PEM');

    if (caKeyPemEnv && caCertPemEnv) {
      try {
        this.caKey  = forge.pki.privateKeyFromPem(caKeyPemEnv);
        this.caCert = forge.pki.certificateFromPem(caCertPemEnv);
        this.caIsEphemeral = false;
        this.logger.log('Loaded CA from env (CA_KEY_PEM/CA_CERT_PEM).');
      } catch (e) {
        this.logger.warn('Failed to parse CA_*_PEM, falling back to ephemeral CA.');
        const ep = makeEphemeralCa();
        this.caKey = ep.caKey; this.caCert = ep.caCert; this.caIsEphemeral = true;
      }
    } else {
      const ep = makeEphemeralCa();
      this.caKey = ep.caKey; this.caCert = ep.caCert; this.caIsEphemeral = true;
      this.logger.log('No CA PEM provided; using ephemeral CA for client cert signing.');
    }
  ...
  /** Sign a CSR with our CA and return a PEM certificate. */
  public signCsr(csrPem: string): string {
    let csr: forge.pki.CertificationRequest;
    try { csr = forge.pki.certificationRequestFromPem(csrPem); } catch { throw new BadRequestException('Invalid CSR PEM'); }
    if (!csr.verify()) throw new BadRequestException('CSR signature invalid');
  ...
  // ======================= existing flows (unchanged except for comments) =======================
  ...
  async generateRegistrationTransaction(
    ...
    const certificatePem = this.signCsr(csrPem); // now using in-process signing
    ...
    // Generate DEK + seal to MXE + upload capsule
    const dek = crypto.randomBytes(32);
    const dekPlaintextB64 = dek.toString('base64');
    ...
    // Upload metadata (with validated location)
    const validLocation = ensureValidLocation(metadata.location, deviceId);
    const fd = metadata.dataTypes[0] || { type: '', units: '', frequency: '' };
    ...
    const fullMeta = {
      ...metadata,
      deviceId,
      location: { latitude: validLocation.latitude, longitude: validLocation.longitude, name: validLocation.name },
      dekPlaintextB64,
      dek_plaintext_b64: dekPlaintextB64,
      dekCapsuleForMxeCid,
      dek_capsule_for_mxe_cid: dekCapsuleForMxeCid,
    };
    const metadataCid = await this.walrusService.uploadMetadata(fullMeta);

    // Build unsigned registration tx
    const ekHash    = Uint8Array.from(metadata.ekPubkeyHash ?? Array(32).fill(0));
    const accessHash= Uint8Array.from(metadata.accessKeyHash ?? Array(32).fill(0));
  ...
  async enrollHardware(
    ...
    const certificatePem = this.signCsr(csrPem);
    const csr = forge.pki.certificationRequestFromPem(csrPem);
    const cnField = csr.subject.getField('CN');
    const deviceId = (cnField?.value as string) || uuidv4().replace(/-/g, '');
    this.logger.log('enrollHardware()', { deviceId, model: metadata.model, deviceName: metadata.deviceName });
  ...
  async getDeviceMetadata(deviceId: string) {
    ...
    if (!dev) throw new NotFoundException(`Device ${deviceId} not found`);
    let walrusMeta: any = null;
    try { walrusMeta = await this.walrusService.getMetadata(dev.metadataCid); } catch {}
    const meta = walrusMeta || dev.metadata || {};
    return {
      deviceId: dev.deviceId,
      metadataCid: dev.metadataCid,
  ...
  async createPayIntent(dto: PayIntentDto) {
    ...
    const lamports = BigInt(
      this.configService.get<string>('REGISTRATION_FEE_LAMPORTS') || `${0.01 * 1e9}`
    );

    const { blockhash } = await this.connection.getLatestBlockhash('confirmed');
    const tx = new Transaction().add(
      SystemProgram.transfer({ fromPubkey: payer, toPubkey: treasury, lamports: Number(lamports) }),
    );
    tx.feePayer = payer;
    tx.recentBlockhash = blockhash;

    const unsignedTxB64 = Buffer.from(tx.serialize({ requireAllSignatures: false })).toString('base64');

    const orderId = uuidv4();
    this.intents.set(orderId, {
      orderId,
      sellerPubkey: dto.sellerPubkey,
      contact: { fullName: dto.fullName, email: dto.email, phone: dto.phone, address: dto.address },
      unsignedTxB64,
    });
